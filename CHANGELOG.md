...
### Changed
- **整理モード「領域別」をコヴィーマトリクス × ヒートマップ形式に刷新**
  - 上部: CSS Grid 2×2 マトリクス。各セルに領域カラーの小ブロック（10×10px）をタスク数分フレックス配置するヒートマップ。`importance=0` のタスクは半透明で区別
  - 各セルをタップすると対応する下部リストのヘッダーへスムーススクロール（`scrollIntoView({ behavior:"smooth" }）`）
  - 軸ラベル（緊急 / 緊急でない）を罫線付きで表示
  - 下部: 第1〜第4を縦並びで常時表示するタスクリスト。0件でも「タスクなし」プレースホルダー付き
  - `scroll-margin-top: 64px` を各見出しに設定し、モーダルのスティッキーヘッダーに隠れないよう配慮

---

## [0.5.10] - 2026-06-21 · Beta

### Added
- **カレンダー提案の取得期間を選択できるプルダウンを追加**（Focus画面・同期ボタン左）
  - 選択肢: 2週間先 / 1ヶ月先 / 3ヶ月先 / 半年先 / 1年先（初期値: 2週間先）
  - 選択値は `localStorage`（キー: `polaris_calendar_timeframe`）に永続化
  - 「カレンダー提案を同期」ボタンを押すと選択期間が GAS に渡される
- **GAS `handleGenerateCalendarTasks_` が `timeframe` パラメータを受け取り終了日を動的計算**（14/30/90/180/365 日後）
- **長期間（30日超）のプロンプトに戦略的予定フィルタ指示を自動付加**：日常の細かな予定を除外し、経営拡張・家族の重大イベントなど準備に数ヶ月を要する予定のみ抽出するよう Gemini に指示

---

## [0.5.9] - 2026-06-20 · Beta

### Fixed
- **SwipeRow iOS長押しスナップショット干渉を2段構えで根本解消**
  - **視覚的ロック** — 赤い削除背景を初期状態 `opacity:0` に設定し、スワイプ閾値（15px）を超えた瞬間にのみ `opacity:1` へ切り替え（`isSwiping` state）。長押し時点では赤背景が描画されないためiOSにキャプチャされない
  - **時間的ロック** — `onTouchStart` で200msタイマー起動。15px到達前にタイムアウトしたら `isLpLocked` refをtrueに設定し、以降の移動計算を完全無視。15px到達時は `clearTimeout` でタイマー解除
  - **CSS** — `WebkitUserDrag:"none"` を追加してiOSネイティブドラッグプレビューを明示的に禁止
...
### Added
- **タスクキャッシュ（爆速起動）** — `localStorage['polaris_tasks_cache']` にタスクリストを永続化。アプリ起動時にAPIを待たず即座にキャッシュをレンダリング。バックグラウンドでGASから最新を取得し、取得完了後にUIとlocalStorageを静かに上書き
- **スピナー表示の最適化** — キャッシュがある場合はフルスクリーンスピナーを表示しない（`showSpinner=false` でバックグラウンドフェッチ）。初回起動（キャッシュなし）の場合のみスピナーを表示
- **tasks 変化時の自動キャッシュ同期** — `useEffect([tasks])` で tasks が変わるたびにlocalStorageへ自動保存。追加・完了・削除・AI提案承認など全ミューテーションが反映される
- **「🔄 Googleタスクを手動同期」ボタン** — 北極星設定モーダル内（AI取得ボタンの直下）に配置。押下でfetchTasksを強制実行し、UIとlocalStorageを最新状態に更新
